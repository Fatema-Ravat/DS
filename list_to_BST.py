...
class BinarySearchTree:
    ...
    def __sorted_list_to_bst(self, nums, left, right):
        if left > right:
            ...
            root_node = Node(nums[left])
        elif right-left == 1:
            root_node = Node(nums[right])
            root_node.left = Node(nums[left])
        else:
            mid_index = left + ((right-left)//2)
            if mid_index<0:
                return None
            root_node = Node(nums[mid_index])
            root_node.left = self.__sorted_list_to_bst(nums,left,mid_index-1)
            root_node.right = self.__sorted_list_to_bst(nums,mid_index+1,right)
        return root_node
